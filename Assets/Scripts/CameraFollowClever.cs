...
public class CameraFollowClever : MonoBehaviour
{
    ...
    void LateUpdate(){
        ...
        if( doRotate ){
            ...
            float wantedRotationAngle = target.eulerAngles.y + 150;
        ...
        //transform.position = target.position + targetAdjustedPosition;
 		transform.position = (target.position + targetAdjustedPosition) - (target.position + targetAdjustedPosition - transform.position) * 0.9f;
